...
const DFS = (graph, informTime, employeeId) => {
  const subordinates = graph[employeeId];

  // edge case
  if (subordinates.length === 0) return 0;

  const numOfMinutes = informTime[employeeId];
  let max = 0;
  ...
  for (let i = 0; i < subordinates.length; i++) {
    ...
    max = Math.max(DFS(graph, informTime, nextSubordinate), max);
  }

  return numOfMinutes + max;
};

const getAdjencyList = (managers) => {
  const adjencyList = managers.map(() => []);
